...
export default function ModalNewsUpdate(props: any) {
  ...
  useEffect(() => {
    setContent(props.record.content);
    setImg(props.record.image);
    form.setFieldValue("title", props.record.title);
    form.setFieldValue("slug", props.record.slug);
    form.setFieldValue(
      "tags",
      props.record.tags.map((k: any) => ({ value: k._id, label: k.name }))
    );
    form.setFieldValue("status", props.record.status);
    form.setFieldValue("date", dayjs(new Date(props.record.date)));
  }, [props.record]);
  return (
            ...
            if (img?._id) {
              await requestToken({
                ...
                data: {
                  ...
                  tags: data.tags.map((k: { value: any }) => k.value),
                },
              });
